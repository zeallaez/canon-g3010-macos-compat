...
之间转换。命令行由 `scanimage` 输出文件；macOS 图形应用则由 AirSane
把同一个 SANE 设备转换成系统内置扫描客户端支持的标准 eSCL/AirScan。
...
命令行模式从同一容器的 `scanimage` 进入，并把 JPEG、PNG 或 TIFF 直接
写入用户选择的 Mac 目录。

容器让扫描运行环境可以重复构建，不需要改动 macOS 系统框架，也不用安装
未签名的 ICA 插件。图形桥接只挂载生成的配置文件；命令行模式另外只挂载
用户指定的输出目录，不会把 Mac 其他文件暴露给扫描运行环境。
...
图形桥接由 Mac 发布
`Canon G3010 Scanner._uscan._tcp.local.`，TXT 记录中声明 `rs=eSCL`，
专用代理主机 `canon-g3010-bridge.local.` 的端点为
`127.0.0.1:8090`。“图像捕捉”发现 Bonjour 服务后，通过 Mac 回环接口
发送 eSCL 请求。用户级登录启动项会在登录后保持桥接和服务记录在线。
