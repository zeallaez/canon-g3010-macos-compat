...
后台还会通过普通 TCP 端口检查打印机是否仍在局域网，而不查询可能正忙的 WSD
端点。连续三次不可达后会撤下失效的 `_uscan` 广播；打印机重新上线时，监督
进程会重新发现地址并自动发布扫描服务。eSCL 作业正在处理时会暂停离线判断，
因此长时间扫描不会被错误中断。

重新发布后，桥接会终止当前用户残留的 `icdd` 发现进程。macOS 的 launchd
会立即重新创建它，已经打开的“图像捕捉”窗口也会自动重连。之所以需要这样
处理，是因为打印机完全断电后，即使新的 eSCL 端点已经健康，“图像捕捉”仍
可能保留断电前失败的 AirScan 会话并报告 `-21345`。
